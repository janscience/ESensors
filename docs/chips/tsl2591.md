...
supporting the TSL2591 within the
[Sensors](https://github.com/janscience/Sensors) framework.  It is
based on the [TSL2591MI](https://bitbucket.org/christandlg/tsl2591mi/)
library.
...
whether it is too small or large and the gain of the sensor is
increased or decreased accordingly.

The sensitivity of the sensor depends also on temperature. You should
use a temperature sensor and provide the temperature to the
`LightTSL2591` device via the `setTemperature()` function in `loop()`.
See the [sensors example](../../examples/sensors/).
...
To get a
[photometric](https://github.com/janscience/Sensors/tree/main/docs/parameters/light#units)
...
diodes have a much wider spectral sensitivity. One common solution is
...
spectrum for IR, as shown in the figure.  Knowing the spectrum of the
measured light source, the appropriately weighted IR signal can then
be subtracted from the full sepctrum signal to get an estimate of
photometric illuminance measured in lux.
...
![IRratio](https://latex.codecogs.com/svg.image?\large&space;\text{IR&space;ratio}&space;=&space;\frac{\text{C1DATA}}{\text{C0DATA}})
...
![irradiancefull](https://latex.codecogs.com/svg.image?\large&space;E&space;=&space;\text{C0DATA}&space;\frac{400&space;\cdot&space;100\text{ms}}{R_e&space;\cdot&space;g&space;\cdot&space;\tau})
...
![irradiance](https://latex.codecogs.com/svg.image?\large&space;E&space;=&space;\frac{\text{C0DATA}}{c_R&space;\cdot&space;g&space;\cdot&space;\tau})
...
![cpl](https://latex.codecogs.com/svg.image?\large&space;\text{cpl}&space;=&space;\frac{\tau&space;\cdot&space;g}{GA&space;\cdot&space;DF})
...
![lux1](https://latex.codecogs.com/svg.image?\large&space;E_1&space;=&space;\frac{\text{C0DATA}&space;-&space;2&space;\cdot&space;\text{C1DATA}}{\text{cpl}})
...
![lux2](https://latex.codecogs.com/svg.image?\large&space;E_2&space;=&space;\frac{0.6&space;\cdot&space;\text{C0DATA}&space;-&space;\text{C1DATA}}{\text{cpl}})
...
![general](https://latex.codecogs.com/svg.image?\large&space;\begin{array}{rcl}&space;E_1&space;&&space;=&space;&&space;\frac{\text{C0DATA}&space;-&space;c_B&space;\cdot&space;\text{C1DATA}}{\tau&space;\cdot&space;g}&space;\cdot&space;\text{GDF}&space;\\&space;&space;E_2&space;&&space;=&space;&&space;\frac{c_C&space;\cdot&space;\text{C0DATA}&space;-&space;c_D&space;\cdot&space;\text{C1DATA}}{\tau&space;\cdot&space;g}&space;\cdot&space;\text{GDF}&space;\\&space;&space;E&space;&&space;=&space;&&space;\max{(E_1,&space;E_2,&space;0)}\end{array})
...
![Erearranged](https://latex.codecogs.com/svg.image?\large&space;E_2&space;=&space;\frac{1&space;-&space;\frac{c_D&space;\cdot&space;\text{C1DATA}}{c_C&space;\cdot&space;\text{C0DATA}}}{\tau&space;\cdot&space;g}&space;\cdot&space;c_C&space;\cdot&space;\text{C0DATA}&space;\cdot&space;\text{GDF})
...
![Esimple](https://latex.codecogs.com/svg.image?\large&space;E&space;=&space;\frac{\alpha}{\tau&space;\cdot&space;g}&space;\cdot&space;\text{C0DATA})

For a given light source, illuminance is simply proportional to the raw count reading.
The factor &alpha; could be calibrated by a single measurement!
