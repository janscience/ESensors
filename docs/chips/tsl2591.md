...
plain sensor in sunlight. Somebody wit h a lux-meter should measure
them.
...
![IR ratio](https://latex.codecogs.com/svg.image?\large&space;\text{IR&space;ratio}&space;=&space;\frac{\text{C1DATA}}{\text{C0DATA}})
...
the IR ratio is about 0.25 for sunlight, and about 0.2 for incandescent light.

Note, however, that the two channels in different ways dependent on
temperature. Without temperature compensation, the IR ratio of a given
light source will quite strongly depend on temperature.
...
that have been measured with maximum gain (x9876) and also 100ms
...
columns show the respective ratios relative to the 100ms setting:
...
combine and scale the two sensor channels.
...
weight the counts obtained from the two channels and scale the overall
result.
...
An earlier version of the [Adafruit TSL2591
  ...
  this approach (it is still as a comment in the code), with
...
Since the IR ration is approximately constant we can summarize all factors into a single parameters &alpha; and get
